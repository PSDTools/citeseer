...
<div class="p-6 lg:p-8">
	<div class="mb-8">
		<a href="/datasets" class="text-sm text-white/40 hover:text-white/70 mb-3 inline-flex items-center gap-1.5 transition-colors">
	...
	<!-- Schema -->
	<div class="mb-8">
		<h2 class="text-lg font-semibold text-white mb-4">Schema</h2>
				...
				<tbody class="divide-y divide-white/5">
					{#each columns as col}
						<tr class="hover:bg-white/[0.02] transition-colors">
							...
							<td class="px-4 py-3">
								{#if col.isTimestamp}
									<span class="rounded-md bg-blue-500/20 px-2 py-0.5 text-xs text-blue-400">timestamp</span>
								{:else if col.isMetric}
									<span class="rounded-md bg-green-500/20 px-2 py-0.5 text-xs text-green-400">metric</span>
								{:else if col.isEntityId}
									<span class="rounded-md bg-purple-500/20 px-2 py-0.5 text-xs text-purple-400">entity_id</span>
								{:else if col.isCategorical}
									<span class="rounded-md bg-amber-500/20 px-2 py-0.5 text-xs text-amber-400">categorical</span>
								{:else}
				...
				<thead class="border-b border-white/10 bg-white/[0.02]">
					<tr>
						{#each columns.slice(0, 8) as col}
							<th class="px-4 py-3 text-left font-medium text-white/40 whitespace-nowrap">{col.name}</th>
						{/each}
						{#if columns.length > 8}
...
<Modal open={showNormalizeModal} onclose={() => { if (!isNormalizing) { showNormalizeModal = false; normalizeResult = null; } }} maxWidth="max-w-md">
	{#if normalizeResult}
		<div class="text-center">
			{#if normalizeResult.success}
				...
				<h3 class="text-lg font-semibold text-white mb-2">Normalization Complete</h3>
				<p class="text-white/50 mb-4">{normalizeResult.message}</p>
			...
			<button
				...
				class="rounded-lg bg-white/10 px-4 py-2 text-sm text-white hover:bg-white/20 transition-colors"
			>
				Close
			</button>
		</div>
	{:else if isNormalizing}
		<div class="text-center py-4">
			...
			<h3 class="text-lg font-semibold text-white mb-2">Analyzing with AI...</h3>
			<p class="text-white/50 text-sm">Detecting date columns and normalizing values</p>
		</div>
	{:else}
		...
		<h3 class="text-lg font-semibold text-white mb-2">Normalize Date Columns</h3>
