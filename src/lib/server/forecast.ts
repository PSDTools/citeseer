...
export async function applyForecastToResult(options: {
	panel: PanelSpec;
	result: QueryResult;
	question: string;
	selectStrategy?: (context: ForecastSelectionContext) => Promise<ForecastStrategyDecision | null>;
}): Promise<ForecastOutcome> {
	const { panel, result, question, selectStrategy } = options;

	if (!panel.forecast || panel.type !== 'line' || !result.success || result.data.length < 3) {
		return { result };
	}

	const columns = result.columns || [];
	const xField = panel.x ? findMatchingColumn(panel.x, columns) : columns[0];
	const yField = panel.y ? findMatchingColumn(panel.y, columns) : columns[1];
	if (!xField || !yField) {
		return { result };
	}

	const parsedPoints: SeriesPoint[] = [];
	let formatCounts: Record<XFormat, number> = {
		yearmonth: 0,
		date: 0,
		number: 0,
		string: 0
	...
	result.data.forEach((row, index) => {
		const parsed = parseXValue(row[xField], index);
		formatCounts[parsed.format] += 1;
	...
	const format: XFormat = (['yearmonth', 'date', 'number'] as XFormat[]).reduce((best, current) => {
		return formatCounts[current] > formatCounts[best] ? current : best;
	}, 'string');
	...
	const points = parsedPoints
		.filter((p) => (format === 'number' ? typeof p.number === 'number' : !!p.date))
	...
	const cadence = inferCadence(points, format);
	const stats = computeTrendStats(points);
	const seasonality = detectSeasonality(points, cadence);
	const inferredHorizon = resolveForecastSpec(panel)?.horizon ?? defaultHorizon(cadence);

	let decision: ForecastStrategyDecision | null = null;
	const allowedStrategies: ForecastStrategy[] = [
		'auto',
		'linear',
		'drift',
		'moving_average',
	...
	let window = resolveForecastSpec(panel)?.window;
	let alpha = resolveForecastSpec(panel)?.alpha;
	let seasonLength = resolveForecastSpec(panel)?.seasonLength;

	if (strategy === 'auto' && selectStrategy) {
		decision = await selectStrategy({
			question,
			panelTitle: panel.title,
			cadence: cadence.unit,
			defaultHorizon: inferredHorizon,
			stats: {
	...
	if (decision) {
		...
		window = decision.window ?? window;
		alpha = decision.alpha ?? alpha;
		seasonLength = decision.seasonLength ?? seasonLength;
	}

	if (strategy === 'auto') {
		if (seasonality.seasonLength && (seasonality.strength || 0) > 0.4) {
			strategy = 'seasonal_naive';
			seasonLength = seasonality.seasonLength;
		} else if (Math.abs(stats.r2) > 0.5 && points.length >= 6) {
			strategy = 'linear';
		} else if (stats.cv > 0.25 && points.length >= 8) {
			strategy = 'exp_smoothing';
		} else {
			strategy = 'moving_average';
		}
	}

	const horizon = decision?.horizon ?? inferredHorizon;
	...
	const resolvedConfidence =
		decision?.confidence ??
		(points.length >= 100 && stats.cv < 0.2
			? 'high'
			: points.length >= 30 && stats.cv < 0.35
				? 'medium'
				: 'low');
	const intervalPct =
	...
	switch (strategy) {
		case 'linear':
			forecastValues = linearForecast(points, horizon);
			break;
		case 'drift':
			forecastValues = driftForecast(points, horizon);
			break;
		case 'exp_smoothing':
			forecastValues = expSmoothingForecast(points, horizon, alpha ?? 0.3);
			break;
		case 'seasonal_naive':
			if (!seasonLength && seasonality.seasonLength) {
				seasonLength = seasonality.seasonLength;
			}
			if (!seasonLength) {
				strategy = 'moving_average';
	...
	for (let i = 1; i <= horizon; i++) {
		...
		if (format === 'number' && typeof lastPoint.number === 'number') {
			nextX = lastPoint.number + cadence.step * i;
		} else if (lastPoint.date) {
			const nextDate = addPeriods(lastPoint.date, cadence, i);
			nextX = format === 'yearmonth' ? formatYearMonth(nextDate) : formatDate(nextDate);
		} else {
			continue;
		}

		const interval = residualStd * intervalZ * Math.sqrt(i);
		forecastRows.push({
			[xField]: nextX,
	...
	const combined = sortRowsByX([...actualRows, ...forecastRows], xField, format);
	const combinedColumns = columns.includes(seriesField) ? columns : [...columns, seriesField];
	const withIntervals = [...combinedColumns];
	if (!withIntervals.includes(LOWER_FIELD)) withIntervals.push(LOWER_FIELD);
	if (!withIntervals.includes(UPPER_FIELD)) withIntervals.push(UPPER_FIELD);

	const finalDecision: ForecastStrategyDecision = decision
		? {
				...decision,
